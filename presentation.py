...
class PolynomialBuilder(object):
    ...
    def get_results(self):
        ...
        phi_strings = ['Phi^{0}_[{1}]={result} - 1\n'.format(i + 1, j + 1, result=self._print_phi_i_j(i, j))
                       for i in range(self._solution.Y.shape[1])
                       for j in range(3)]
        f_strings = ['F^{0} в особом базисе:\n{result} - 1\n'.format(i + 1, result=self._print_F_i(i))
                     for i in range(self._solution.Y.shape[1])]
        f_strings_transformed = [
            'F^{0} в обычном базисе:\n{result} - 1\n'.format(i + 1, result=self._print_F_i_transformed(i))
            for i in range(self._solution.Y.shape[1])]
        f_strings_transformed_denormed = ['F^{0} в стандартном базисе '
                                          'денормированный:\n{result}\n'.format(i + 1, result=
    ...
    def plot_graphs(self):
        ...
        for index in range(self._solution.Y.shape[1]):
            ...
            ax.plot(np.arange(1, self._solution.n + 1), self._solution.Y_[:, index],
                    'b-', label='$Y_{0}$'.format(index + 1))
            ax.plot(np.arange(1, self._solution.n + 1), self._solution.F_[:, index],
                    'g-', label='$F_{0}$'.format(index + 1))
            ax.legend(loc='upper right', fontsize=16)
            ax.set_title('График {0}'.format(index + 1))
            ax.grid()

            norm_ax.set_xticks(np.arange(0, self._solution.n + 1, 5))
            norm_ax.plot(np.arange(1, self._solution.n + 1),
                         abs(self._solution.Y_[:, index] - self._solution.F_[:, index]), 'k-')
            props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
            norm_ax.text(1, 2, 'норм. невязка(max):'+str(self._solution.norm_error[index])+'\nнорм. невязка(avg)'+str(self._solution.norm_error_a[index]), fontsize=11, verticalalignment='top',bbox = props)
            norm_ax.set_title('Невязка {0}'.format(index + 1))
            norm_ax.grid()
...
class PolynomialBuilderExpTh(PolynomialBuilder):
    ...
    def get_results(self):
        ...
        f_strings = ['F^{0} в особом базисе:\n{result}\n'.format(i + 1, result=self._print_F_i(i))
                     for i in range(self._solution.Y.shape[1])]
        f_strings_transformed = ['F^{0} в стандартном базисе:\n{result}\n'.format(i + 1, result=self._print_F_i(i, mode=1))
                                 for i in range(self._solution.Y.shape[1])]
        f_strings_transformed_denormed = ['F^{0} в стандартном базисе '
